    ...
    <section>
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold text-slate-950">留资管理</h1>
          <p className="mt-3 text-slate-600">查看预约沉淀的客户留资，维护后续跟进备注。</p>
        </div>
      ...
      <form className="mt-6 flex flex-col gap-3 rounded-lg bg-white p-4 shadow-sm sm:flex-row">
        <input
          name="keyword"
          defaultValue={keyword}
          placeholder="搜索姓名 / 手机号 / 公司名称"
        ...
        {keyword ? (
          <Link href="/admin/leads" className="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700">
            清空
          </Link>
        ) : null}
      </form>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {leads.map((lead) => (
                <tr key={lead.id}>
                  <td className="whitespace-nowrap px-4 py-3 font-medium text-slate-950">{lead.contactName}</td>
                  ...
                  <td className="whitespace-nowrap px-4 py-3">
                    <Link
                      href={`/admin/leads?phone=${encodeURIComponent(lead.contactPhone)}${keyword ? `&keyword=${encodeURIComponent(keyword)}` : ""}`}
      ...
      {selectedPhone ? (
        <section className="mt-8 rounded-lg bg-white p-6 shadow-sm">
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
            <div>
              <h2 className="text-xl font-semibold text-slate-950">客户预约记录</h2>
              <p className="mt-1 text-sm text-slate-600">手机号：{selectedPhone}</p>
            </div>
            <Link href="/admin/leads" className="text-sm font-semibold text-blue-700 hover:text-blue-900">
              关闭
            </Link>
          </div>
