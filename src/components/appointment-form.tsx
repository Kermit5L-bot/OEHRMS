...
import { CalendarDays, CheckCircle2, MessageSquareText, Send, UserRound } from "lucide-react";
import {
  customerTypeOptions,
  getTodayDateString,
  interestAreaOptions,
...
export function AppointmentForm({ showrooms, initialShowroomId }: AppointmentFormProps) {
  ...
  function validate() {
    ...
    if (!form.internalContactInfo.trim()) nextErrors.internalContactInfo = "请填写对接人";
    ...
    <form onSubmit={handleSubmit} className="glass-panel rounded-lg p-5 sm:p-6">
        ...
        <Field label="预约展厅" error={errors.showroomId} required>
          <FormSelect
            ...
            options={showrooms.map((showroom) => ({
              ...
              label: `${showroom.name}${showroom.status !== "open" ? "（暂停预约）" : ""}`,
              disabled: showroom.status !== "open",
            }))}
          />
        </Field>
      ...
      <FormGroup title="联系人信息" icon={UserRound}>
        ...
        <Field label="手机号码" error={errors.contactPhone} required>
          ...
          <p className="mt-1 text-xs text-cyan-100/75">预约成功后会收到短信提醒</p>
        </Field>
        ...
        <Field label="对接人" error={errors.internalContactInfo} required>
          <input
              ...
              <label
                ...
                className={`flex min-h-11 items-center gap-2 rounded-md border px-3 text-sm font-semibold transition ${
                  form.interestAreas.includes(option.value)
                    ? "border-emerald-300 bg-emerald-400/18 text-white shadow-[0_0_0_1px_rgba(52,211,153,0.25),0_10px_24px_rgba(5,150,105,0.16)]"
                    : "border-cyan-300/20 bg-slate-950/30 text-slate-100"
                }`}
              >
                <input
                  type="checkbox"
      ...
      {options.map((option) => (
        <label
          ...
          className={`flex min-h-11 items-center justify-center gap-2 rounded-md border px-3 text-sm font-semibold transition ${
            value === option.value
              ? "border-emerald-300 bg-emerald-400/18 text-white shadow-[0_0_0_1px_rgba(52,211,153,0.25),0_10px_24px_rgba(5,150,105,0.16)]"
              : "border-cyan-300/20 bg-slate-950/30 text-slate-200"
          }`}
        >
          <input
            type="radio"
            ...
            className="sr-only"
          />
          {value === option.value ? <CheckCircle2 className="h-4 w-4 text-emerald-300" /> : null}
          {option.label}
        </label>
      ))}
    </div>
